...
6. **Rail/list drag interactions need a taller viewport.** Scroll-box rails like the parts list are short (~55px tall with a few rows) at the default 1280×900, so drag-target rows can sit scrolled out of the clipped area and the drag silently no-ops. Bump to something like 1400px tall for any spec that drags rows within a rail/list.
