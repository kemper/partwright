...
6. **Rail/list drag targets need a tall viewport.** Short scrollable rails (e.g. the parts list) are only ~55px tall at the default 1280×900 viewport with a handful of items — drag-and-drop e2e can silently miss because rows are clipped out of view, not because the drag logic is wrong. Call `page.setViewportSize({ width: 1280, height: 1400 })` (or similar) in tests that drag within a short scroll container.
