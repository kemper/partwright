...
2. **Viewport size.** The default Desktop Chrome viewport (1280×720) clips the AI panel's toggle strip. The config sets 1280×900 — keep it that way for anything that interacts with elements in the bottom half of the panel. Scrollable list/rail widgets (e.g. the parts list) can render only a couple of visibly-tall rows at that height, so drag-and-drop specs against them silently miss targets that are scrolled out of the clip — bump `page.setViewportSize` taller (~1400px) for any test that drags within a short scroll box.
