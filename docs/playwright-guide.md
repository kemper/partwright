...
6. **Rail/list drags need a taller viewport.** Scrollable lists (the parts rail, layer lists) render as a short clipped box at the default 1280×900 — as little as ~55px tall with several rows scrolled out of view. A drag-and-drop spec against one of these needs a taller viewport (e.g. `test.use({ viewport: { width: 1280, height: 1400 } })`) or the target rows sit outside the interactable area and the drag silently no-ops instead of failing loudly.
