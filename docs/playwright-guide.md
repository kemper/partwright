...
6. **Rail/list drag tests need a taller viewport.** A short scroll box (e.g. the parts list) can be only ~55px tall with a few rows at the default 1280×900 viewport, silently clipping drag targets out of view — the drag then no-ops with no error. Set a taller viewport (`page.setViewportSize({ width: 1280, height: 1400 })` or similar) for any spec that drags within a scrollable list/rail.
