...
6. **Rail/list drag targets need a tall viewport.** Scrollable side-rail lists (e.g. the parts list) are short at the default 1280×900 — only a few rows are visible before the rest scroll out of the clip, so a drag-and-drop test can silently target/miss the wrong row. Bump `page.setViewportSize` (or the test's own viewport) to ~1400px tall for any spec that drags within such a list.
