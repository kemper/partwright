...
6. **Rail/list drag tests need a tall viewport.** Scrollable side-panel lists (e.g. the parts list) render as a short clipped box at the default 1280×900 — only a few rows are visible, so drag-and-drop tests silently miss any row that's scrolled out of the clip instead of failing loudly. Set a taller viewport (`page.setViewportSize({ width: 1280, height: 1400 })` or similar) for any spec that drags within a scrollable rail/list.
