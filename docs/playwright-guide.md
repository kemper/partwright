...
6. **Rail/list drag interactions need a taller-than-default viewport.** The parts-list is a short scroll box even at the config's 1280×900 (~55px tall with a few parts) — drag targets below the fold are silently missed, not failed, because Playwright can still resolve a locator's bounding box outside the visible clip. If a test drags rows in the parts list (or any similar scrollable rail), bump the test's viewport height (~1400px) rather than trusting the global default.
