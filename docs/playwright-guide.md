...
6. **Rail/list drags need a tall viewport.** Scrollable rails (e.g. the parts list) are only ~55px tall with a few items at the default 1280×900 viewport, so drag-and-drop targets can be scrolled out of the clip region and silently miss — the drag no-ops with no error. Bump to a taller viewport (e.g. 1400px) for any spec that drags within a short scroll box.
