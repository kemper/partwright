...
6. **Rail/list drag tests need a tall viewport.** Scrollable lists (e.g. the parts rail) render only a handful of rows at the default 1280×900 — drag targets below the fold are silently unreachable, not just slow. Bump `viewport` to something like 1280×1400 for any spec that drags within a scrollable list.
