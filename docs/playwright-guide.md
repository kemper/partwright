...
6. **Rail/list drag tests need a tall viewport.** Lists like the parts panel are short scroll boxes at the default 1280×900 viewport — with only a few rows visible, drag targets below the fold silently fail to hit. Bump `viewport: { height: 1400 }` (or similar) for any spec that drags within a scrollable rail/list, rather than debugging "why didn't the drop land."
