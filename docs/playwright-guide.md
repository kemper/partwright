...
6. **Rail/list drag interactions need a tall viewport.** A short scroll box (e.g. the parts list) can render as little as ~55px tall at the default 1280×900 size, leaving most rows scrolled out of the clip rect — a drag-and-drop e2e test will silently miss those rows instead of failing loudly. Bump the test's viewport height (e.g. 1400px) for any spec that drags within a scrollable rail/list.
