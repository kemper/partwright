...
6. **Rail/list drag tests need a tall viewport.** Lists like the parts rail render as a short scroll box at the default 1280×720 viewport — a few rows can fill the visible ~55px and leave the drag target scrolled out of the clip, so drag-and-drop assertions silently no-op instead of failing loudly. Bump the test's viewport (e.g. `page.setViewportSize({ width: 1280, height: 1400 })`) for any spec that drags rows within a scrollable list.
