...
6. **Rail/list drags need a tall viewport.** The default 1280×900 viewport clips a short scroll box (e.g. the parts list) to a handful of rows — drag-to-target locators on rows below the fold silently miss instead of failing loudly. If a spec drags within a scrollable rail/list, bump `page.setViewportSize` to something like 1400px tall for that test rather than debugging "the drop never registered."
