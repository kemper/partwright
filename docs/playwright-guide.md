...
6. **Rail/list drag targets need a tall viewport.** Short scroll boxes (e.g. the parts-list rail) clip rows below the fold at the default 1280×900 — drag-and-drop e2e against them silently misses with no error, because the target is offscreen, not absent. Bump the test's viewport (`page.setViewportSize({ width: 1280, height: 1400 })` or similar) for any spec dragging within a scrollable list.
