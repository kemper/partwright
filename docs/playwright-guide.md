...
6. **Rail/list drags need a tall viewport.** Any e2e test that drags rows in a scrollable list (parts list, layer list, …) by absolute position needs every target row actually in view — at the default 1280×900 a short list clips to a handful of visible rows, and drags targeting scrolled-out rows silently no-op instead of failing loudly. Call `page.setViewportSize({ width: 1280, height: 1400 })` (or taller) in that spec's `beforeEach`; see `tests/part-grouping.spec.ts` for the pattern.
