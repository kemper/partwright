    ...
    <div class="flex flex-col gap-5">
      ...
      <Section title="UI">
        ...
        <Field
          ...
          tooltip="The Worker Health panel (Workers button in the ⚠ Diagnostic Log, or 'Show worker health' in the command palette) keeps the last N geometry runs in memory with their wall-clock and worker-side timing. Older runs are evicted when the buffer is full. Increase to keep more history for spotting slow-render patterns; decrease to reduce memory use."
        ...
        <Field
          ...
          unit="ms"
          hint="How often the worker health panel re-polls in-flight counts while open."
          tooltip="The Worker Health panel polls live values (in-flight operation counts, worker liveness) on this interval, since those change without firing an update event. Lower = snappier live readout, slightly more work while the panel is open. Only affects the panel while it's visible."
