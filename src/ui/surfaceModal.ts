...
export function openSurfaceModal(api: SurfaceApi, initialTab: Tab = 'fuzzy'): void {
  ...
  // change engines, so they stay bake-only (their tabs also hide the region UI).
  const IN_CODE_IDS = new Set<Tab>(['fuzzy', 'knit', 'cable', 'waffle', 'fur', 'woven', 'voronoi', 'smooth']);
  const REGIONLESS_TABS = new Set<Tab>(['voxelize', 'voronoiLamp']);
  ...
  function updateApplyBtn() {
    ...
    pathHint.textContent = asCode
      ...
      : REGIONLESS_TABS.has(active)
        ? 'Converts the model and saves a new version (the parametric code is replaced).'
  ...
  function renderTab() {
    ...
    updateApplyBtn(); // Apply's label + path hint depend on the active tab
    schedulePreview();
  }
