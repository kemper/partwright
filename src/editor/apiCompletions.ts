...
const API_MEMBERS: Completion[] = [
  ...
  fn('alignTo', '(shape, target, {x?, y?, z?})', 'Translate shape so its bbox edges align with target\'s along the given axes (min/max/center).'),
  fn('placeOn', '(shape, target, {at?, gap?})', 'Drop shape so its bottom sits on target\'s top, centered at `at` (or target\'s bbox center).'),
  ...
  fn('circularPattern', '(shape, count, {axis?, angle?, center?})', 'count copies rotated around an axis. Default axis "z", angle 360.'),
  fn('expectUnion', '(parts, {expectComponents?})', 'Boolean union with a component-count check — throws a diagnostic if the result doesn\'t have the expected piece count.'),
