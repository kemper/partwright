...
import { applyFuzzy, applyFuzzyPatch, applyKnit, applyKnitAsync, applyKnitPatch, applyKnitPatchAsync, applyCable, applyCablePatch, applyWaffle, applyWafflePatch, applyFur, applyFurPatch, applyWoven, applyWovenPatch, applyVoronoi, applyVoronoiPatch, applyVoronoiLamp, applyPerforate, applyEngrave, applySmooth, applySmoothPatch, applyVoxelize, applyScale, defaultFuzzyOptions, defaultKnitOptions, defaultCableOptions, defaultWaffleOptions, defaultFurOptions, defaultWovenOptions, defaultVoronoiOptions, defaultVoronoiLampOptions, defaultPerforateOptions, defaultEngraveOptions, defaultSmoothOptions, modelDiagonal, applyTransform, SdfAbortError, type ModifierResult, type EngraveProjection, type StampMask, type SdfRunControl } from './surface/modifiers';
...
async function main() {
  ...
  async function buildSurfaceModifier(
    id: 'fuzzy' | 'knit' | 'cable' | 'waffle' | 'fur' | 'woven' | 'voronoi' | 'voronoiLamp' | 'perforate' | 'engrave' | 'smooth' | 'voxelize',
  ...
  const SDF_HEAVY = new Set(['engrave', 'voronoiLamp', 'perforate']);
  ...
  const partwrightAPI = {
     ...
     *  id: 'fuzzy'|'knit'|'cable'|'waffle'|'fur'|'woven'|'voronoi'|'voronoiLamp'|'perforate'|'engrave'|'smooth'|'voxelize'. */
    async previewSurfaceModifier(id: 'fuzzy' | 'knit' | 'cable' | 'waffle' | 'fur' | 'woven' | 'voronoi' | 'voronoiLamp' | 'perforate' | 'engrave' | 'smooth' | 'voxelize', opts?: Record<string, unknown>, preserveColor = true): Promise<{ ok: true } | { error: string }> {
      try {
    ...
    async applyPerforatedLattice(opts?: {
      ...
      try {
        const preserve = opts?.preserveColor ?? true;
        return await commitSurfaceModifier(await buildSurfaceModifierProgress('perforate', opts, preserve), preserve);
      } catch (e) { return { error: e instanceof Error ? e.message : String(e) }; }
    },
