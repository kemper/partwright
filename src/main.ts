...
function updateDocumentTitle(context?: { page?: 'landing' | 'editor' | 'help' | '404' | 'catalog' | 'legal'; sessionName?: string | null }) {
  let route: RouteName;
...
function shouldShow404(): boolean {
  const path = window.location.pathname;
  return path !== '/' && path !== '' && path !== '/help' && path !== '/editor' && path !== '/catalog' && path !== '/legal';
}
...
async function main() {
  ...
  async function syncRouteFromURL() {
    ...
    if (shouldShowLanding() || shouldShowHelp() || shouldShowCatalog() || shouldShowLegal() || shouldShow404()) {
  ...
  if (!showLanding && !showHelpPage && !showCatalog && !showLegalPage && !show404) {
    maybeStartTour();
    maybeShowShortcutsHint();
  }

  // If not on landing/help/catalog/404, load session or default code now
  if (!showLanding && !showHelpPage && !showCatalog && !showLegalPage && !show404 && engineOk) {
    await syncEditorFromURL();
  }
