...
async function main() {
  ...
  function preloadDeformersForVersion(geometryData: Record<string, unknown> | null): void {
    clearDeformers();
    clearRedoStack(); // redo history is per-version; loading a different version clears it
  ...
  /** Undo the most recently applied deformer. Saves a new version with N-1
   *  deformers; the undone deformer is pushed onto the redo stack. */
  async function undoDeformerAndSave(): Promise<
    { versionId: string | null; undone: { kind: DeformerKind; params: DeformerParamsAny } } | { error: string }
  > {
    if (!hasDeformers()) return { error: 'Nothing to undo — no deformers are applied to this version.' };

    // Snapshot current list so redo can restore it.
    pushRedoSnapshot([...serializeDeformers()]);

    const undone = popLastDeformer()!;

    // Re-run the code to produce the base mesh, then replay remaining deformers.
    ...
    if (getState().session) {
      const thumbnail = await captureThumbnail();
      const geoData = enrichGeometryDataWithDeformers(enrichGeometryDataWithColors(getGeometryDataObj()));
      const saved = await saveVersion(getValue(), geoData, thumbnail, `undo ${undone.kind}`, undefined, { force: true });
      savedId = saved?.id ?? null;
    }

    return { versionId: savedId, undone: { kind: undone.kind, params: undone.params as DeformerParamsAny } };
  }

  /** Redo the most recently undone deformer. Restores the full deformer list
   *  from the redo stack and saves a new version. */
  async function redoDeformerAndSave(): Promise<
    { versionId: string | null; redone: { kind: DeformerKind; params: DeformerParamsAny } } | { error: string }
  > {
    const snapshot = popRedoSnapshot();
    if (!snapshot) return { error: 'Nothing to redo — redo stack is empty.' };

    const redoneDeformer = snapshot[snapshot.length - 1];
    // Restore the full deformer list from the snapshot (bypasses addDeformer's redo-clear).
    setDeformers(snapshot);

    // Re-run the code + replay the restored deformers.
    await runCodeSync(getValue());
    syncLockState();
    ...
    if (getState().session) {
      const thumbnail = await captureThumbnail();
      const geoData = enrichGeometryDataWithDeformers(enrichGeometryDataWithColors(getGeometryDataObj()));
      const saved = await saveVersion(getValue(), geoData, thumbnail, `redo ${redoneDeformer.kind}`, undefined, { force: true });
      savedId = saved?.id ?? null;
    }

    return { versionId: savedId, redone: { kind: redoneDeformer.kind, params: redoneDeformer.params as DeformerParamsAny } };
  }
  ...
  const partwrightAPI = {
    ...
    async undoLastDeformer(): Promise<
      { versionId: string | null; undone: { kind: DeformerKind; params: DeformerParamsAny } } | { error: string }
    > {
      return undoDeformerAndSave();
    },

    async redoDeformer(): Promise<
      { versionId: string | null; redone: { kind: DeformerKind; params: DeformerParamsAny } } | { error: string }
    > {
      return redoDeformerAndSave();
    },
  ...
  function initSculptUndoShortcuts() {
    document.addEventListener('keydown', (e) => {
      if (!(e.ctrlKey || e.metaKey)) return;
      const target = e.target as HTMLElement | null;
      if (target && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.isContentEditable)) return;
      if (!hasDeformers() && getRedoStack().length === 0) return; // nothing sculpt-related to undo/redo
      ...
      e.preventDefault();
      if (isUndo) {
        void undoDeformerAndSave();
      } else {
        void redoDeformerAndSave();
      }
    });
  }
