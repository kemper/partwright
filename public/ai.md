...
| `pointers` | **Review-before-paint workflow** — drop labelled mesh-anchored callouts at surface points you think are a feature (an iris, a foot, a button), the user reviews/corrects them in the Pointer panel, then commit paint against the agreed anchors. The right surface when you're about to paint several features and want to catch part-misidentification BEFORE baking it into 80 paint ops. Covers `dropPointer` / `listPointers` / `previewPointerPaint` / `commitPaintFromPointer(s)` / `getPointerCoverageReport` / `hidePointers` / `showPointers` / `clearPointers`, the three `paintHint` kinds (`connected` / `coplanar` / `colorFlood`), and the stale/orphaned flagging. |
