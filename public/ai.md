...
| Low-poly / faceted creature or prop (incl. low-poly-ing an imported STL) | **Build smooth with `api.sdf`, then crystallize: `api.lowPoly(shape, { targetTriangles })`** — NOT a union of primitives to fake facets -> `/ai/lowpoly.md` | (use manifold-js) | (mesh-only; not in BREP) |
...
| `lowpoly` | Before building a **low-poly / faceted** creature or prop, or low-poly-ing an imported STL — the `api.lowPoly(shape, { targetTriangles })` crystallize step. Build the smooth SDF form first, then decimate + flat-shade it in one call (NOT a union of primitives). Includes coat-painting with `api.paint.pattern`. |
...
#### Low-poly / faceted models — `api.lowPoly`

Crystallize any Manifold into the coarse, flat-shaded **low-poly** look in one call — `api.lowPoly(shape, { targetTriangles, facetSize?, flatShade? })` decimates to a triangle budget (even facet size, preferred) and returns a Manifold tagged for flat/faceted rendering (`flatShade` defaults to `true`). **For a low-poly creature, do NOT union primitives to fake facets** — build the smooth SDF form, then `return api.lowPoly(body.build({ edgeLength: 0.6 }), { targetTriangles: 800 })`, and paint the coat with `api.paint.pattern` (not world-axis boxes, which jag on coarse meshes). Also low-polys imported STLs: `api.lowPoly(Manifold.ofMesh(api.imports[0]), { targetTriangles: 1000 })`. Full guide + budget ranges: `readDoc("lowpoly")`.

```js
const body = api.sdf.capsule([0,0,0],[0,0,20], 6).smoothUnion(api.sdf.sphere([0,0,23], 7), 3);
return api.lowPoly(body.build({ edgeLength: 0.6 }), { targetTriangles: 700 });
```
