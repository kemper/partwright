...
- `src/reconstruct/` — mesh→code reconstruction: pure kernel ported from `scripts/inverse-cad/` (`slice2d.ts` Z-slicing + Douglas-Peucker, `sampleMesh.ts` sampling + k-d tree, `meshDistance.ts` chamfer/hausdorff, `meshComponents.ts` weld/split, `sectionCode.ts` section→levelSet codegen) running in `reconstructWorker.ts`. Powers `partwright.convertToCode()` / `evalAgainstImport()`, the "Convert model to code" palette command, the AI tools of the same names, and the `/reconstruct` chat command (`src/ai/reconstructPrompt.ts`, subdoc `public/ai/reconstruction.md`). Unit tests: `tests/unit/reconstruct.test.ts`; e2e: `tests/reconstruct.spec.ts`.
