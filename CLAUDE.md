...
2. On every push to main, the **`Gate main → staging`** GitHub Action (`.github/workflows/staging-gate.yml`) runs `npm run build`, `npm run test:unit`, and `npm run test:e2e`. **Only if all pass** does it fast-forward `staging` to that commit, which Cloudflare then deploys to the staging preview. A red gate leaves `staging` parked on the last known-good commit. On a red gate, the **`Notify on staging-gate failure`** Action (`.github/workflows/notify-on-gate-failure.yml`) fires via `workflow_run` and opens a `staging-gate-failure`-labeled issue carrying the failed **run ID** (and an `@claude` mention). It deliberately does *not* paste log snippets — an agent picking up the issue fetches the full failed-job logs (`get_job_logs` with `failed_only`) and the run's `playwright-artifacts-shard-*` artifacts on demand, which is more complete than any excerpt. Dedupe is on the run ID, so a re-run won't double-file.
