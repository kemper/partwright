...
>
> **A `smoothUnion` over a fully-labelled figure node wipes ALL paint labels.** Smooth blends can't carry label attribution — `smoothUnion(prop, figure)` where `figure` has labelled regions erases them silently. Fix: hard-union the prop instead (ensure it overlaps the figure by 0.5+ units so it stays one component) while the labels survive.
>
...
> ```
>
> `--require-labels` is the headless twin of `scripts/build-catalog-entry.cjs --require-labels`, so you catch buried-feature paint failures in the fast loop instead of at bake time. **Pass only the labels THIS figure must show** — closed-lid / closed-mouth figures legitimately paint 0 for eyes/teeth, so a blanket gate would false-positive. Note `components` here is the Node SSR count and can still **under-report vs the browser bake** for near-threshold thin features (see the headless-`componentCount` callout above) — trust `figure:smoke` for *paint resolution*, still verify *component splits* in the browser bake. **The colored catalog bake is the only correctness oracle for both `componentCount` and paint-label resolution** — `model:preview` is reliable for proportions and pose, but cannot surface marginal-weld splits or unresolved paint regions that only appear in the browser mesher. Re-bake after any engine change and read the per-entry `components=`/`NOT-MANIFOLD` flags as a regression gate across every catalog pose.
>
> **Hair textures (`coils`/`strands`) alias into disconnected islands at coarse `edgeLength` (≥ ~0.5) in the browser bake.** The coarser tri-budget grid fuses them under SSR but splits them in the browser. Use smooth hair when the tri-budget forces a coarser grid; if you must use coils/strands, keep `edgeLength ≤ ~0.4` for the affected label.
...
When two functions share identical logic (same DOM manipulation, same data transformation), extract the shared part into a single helper and have both callers use it. Copy-pasted logic drifts out of sync when one copy gets updated and the other doesn't. **When fixing a pattern in a shared engine file (`sdfFigure.ts`, etc.), grep `examples/` for copies of the OLD idiom in the same pass** — engine-level fixes don't reach figures that hand-rolled the same geometry inline, and the stale copy won't be caught by any test.
...
- **Brief triage agents on confirmed defects, not open-ended sweeps.** "Find all defects in X" prompts produce high false-positive rates — normal anatomy, intended behavior, and already-fixed code all get flagged. A tight "check these specific angles for these specific known defects — FIXED/PARTIAL/BROKEN" brief is faster and far more reliable. Always verify agent triage reports against the actual rendered output before acting on them.
- **Check whether a capability already exists before building it.** Before implementing a new tool, script, or daemon, read the relevant CLAUDE.md sections and `docs/headless-cli.md`. The warm-browser daemon (`bin/partwright.mjs daemon`) and the `compare`/`iterate` commands have been re-requested after they shipped; the colored-preview path is already the default.
