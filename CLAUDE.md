...
15. **STL import**: Click Import → "Choose file…" → pick an `.stl`. With no real session content open (a fresh `/editor`), a new session is created named after the file. With a session that has saved work, an **import-target modal** appears first: choose **New part** (default — adds a separate part to the current session), **Add to current part** (composes the mesh with the part's existing geometry via `Manifold.compose`), or **New session**. In all cases the editor shows a short `return Manifold.ofMesh(api.imports[0])` wrapper (or a `Manifold.compose([...])` form when combined), the mesh renders in the viewport, and the version label is "imported". Editing the wrapper (e.g. adding `.subtract(Manifold.cube([5,5,5], true))`) re-renders correctly. Closing and reopening the session must restore the imported mesh from IndexedDB.
