...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a` or `rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool has no binary-mode flag** (its `-a` doesn't exist in its schema) — reach for **Bash** `grep -an` (or `rg -a`) directly, not the `Grep` tool, for any `main.ts` search.
...
If a grep on `main.ts` returns nothing for a symbol you expect to find there, binary-detection is the first thing to check. Four independent sessions have each spent turns re-discovering this — most recently one that reached for the `Grep` tool's `-a` before falling back to Bash.
