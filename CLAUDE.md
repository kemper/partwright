...
>
> **`model:preview` shades by face normal — it does NOT show `api.label`/palette colors** (in-code `api.paint.*` ops DO resolve and show). To confirm color correctness (e.g. "is this the boot or the skin showing?"), you must render the **colored** catalog bake: `xvfb-run -a node scripts/build-catalog-entry.cjs --source <file> --lang manifold-js --out /tmp/x.partwright.json --palette-file <palette>` writes `/tmp/x.thumb.png`; point its camera with `THUMB_AZIMUTH`/`THUMB_ELEVATION` (`THUMB_AZIMUTH=0` = front/−Y; negative elevation = from below). A shaded-normal preview that "looks solid" can still be a different label's surface. **Paint regions need ≳1 triangle of relief to avoid dithering** — the bake's nearest-centroid label transfer can't cleanly resolve two coincident surfaces, so a totally flush painted region halos/hatches. "More flush" is harder to paint, not easier; the fix is a slight proud protrusion (~1 mesh triangle thick, local `faceDetail` sphere to keep the mesh fine there). **Named views (`--views front`) are reliable for model:preview; `--view 0,el` is NOT necessarily the front face** — the CLI rasterizer uses a different azimuth origin than `multiview.ts`. Use `--views front,iso,right,back` for reliable anatomical orientations.
>
> **Measure geometry empirically when `smoothUnion` is involved — don't trust analytic primitive extents, and never assert surface positions from `bounds()`.** A `smoothUnion` bulges the surface *past* either input primitive's bounds (a foot's real underside sat ~0.79·r below the sole centre vs the analytic 0.65·r), and **`SdfNode.bounds()` always returns a loose conservative AABB**, not the true surface extent — assertions against it will be wrong by the blend halo. To find a true surface position, walk `evaluate(x,y,z)` along the axis in a tiny vite-node script (`npx vite-node probe.mjs` importing `__figureTestables__` + `__testables__`) until the sign flips. Set clearances/clip planes below the *measured* value, then confirm coverage with a sample-grid check (sample where one label is solid; assert the covering label is solid there too).
>
> **For SUBJECTIVE / aesthetic work, prototype options and get the user's pick BEFORE wiring it into a builder.** When the deliverable is *how something looks* (a shoe/sole, a face, a silhouette, a colour scheme) — not a measurable spec — don't implement one interpretation and iterate it through full implement→bake→review cycles; that's the slowest path and it burns the user's patience. Instead build **2–3 throwaway variations**, render them **from ≥4 angles (incl. the underside) in colour**, show the user a side-by-side (e.g. `bin/partwright.mjs compare`, or `--view "az,el;az,el;…"` / colored bakes montaged with `sharp`), and let them choose the direction. One comparison round beats five blind iterations. (This is the lesson from the footwear sole: several rounds shipped "fixed" before a quick demo-and-pick converged it.) **When a fix touches a shared helper that one figure forks inline, ask up front whether the target is the helper or just that one figure** — anchoring on the literal artifact and patching it locally is the wrong altitude if the engine is the root. One scoping question beats a patch-then-revert cycle.
>
> **Long multi-figure catalog bakes in a remote container must run in the foreground** — a background bake fired with `run_in_background` can be silently reaped when the container idles (dev server killed, bake mid-run, no notification). Run a full-catalog rebake interactively while the user is present, or at minimum check the background job explicitly before reporting it done — don't fire-and-forget a 20-minute bake.
...
> **`scripts/build-catalog-entry.cjs` requires a visible display AND a running Vite dev server.** For the display: `xvfb-run -a node scripts/build-catalog-entry.cjs <entry.js> <out.json> --palette palette.json`. If the script exits immediately with a Chromium launch error, `xvfb-run` is the fix. **For the dev server: run `npm run dev` in a separate terminal first** — the script connects to `:5173` and does NOT start one automatically. The error `FAIL [init]: API never appeared` means no dev server is listening. `catalog-regen.cjs` uses `headless: true` and does NOT need a dev server; it does thumbnail-only rebakes (no geometry re-bake). `public/catalog/*.thumb.png` files are **untracked sidecars** (the real thumbnail is the data-URL embedded in `.partwright.json`); they are intentionally not committed — ignore Stop-hook noise about them.
...
- **Git is single-writer.** The working tree and index are shared mutable state. Don't run git mutations while a subagent is also touching the same checkout. Resolve merges/rebases inline yourself; if you must delegate git work to a subagent, give it an isolated worktree (`isolation: "worktree"`). **Don't parallel-edit the same source file across isolated worktrees** — stash/checkout races leak working-tree state between agents editing the same large file concurrently. If multiple agents must edit one file (e.g. `sdfFigure.ts`), give each agent DISTINCT non-overlapping functions; review each diff before cherry-pick integrating.
