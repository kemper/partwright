...
15. **STL import**: Click Import → "Choose file…" → pick an `.stl`. A new session is created named after the file, the editor shows a short `return Manifold.ofMesh(api.imports[0])` wrapper, and the mesh renders in the viewport. The version label is "imported" and editing the wrapper (e.g. adding `.subtract(Manifold.cube([5,5,5], true))`) re-renders correctly. Closing and reopening the session must restore the imported mesh from IndexedDB. Heavy meshes (> 20k triangles) first show an "Import detail" modal with a precise "Max triangles" target (numeric input + slider, backed by a tolerance search over `simplify`) that reduces the mesh toward that count; reduced imports get the label "imported · reduced" and a `[IMPORT]` session note. Imported meshes are exempt from the global "Mesh detail" refine factor (in the viewport Mesh popover), so their triangle count is preserved on import.
16. **Mesh settings (per session)**: The viewport `⬢ Mesh` button (next to Paint / Annotate / Measure) opens a popover with Curve quality (Low → Ultra, default Very High = 128 segments) and Mesh detail (refine factor, default off). These are **session-scoped**, not a global localStorage preference: changing them persists to the active session (`session.meshSettings`, schema 1.7), restores on reload/reopen, travels on export/import, and resets to defaults for a brand-new session.
