...
- **`grep`/`rg` silently truncate results** or skip the file. The dedicated `Grep` tool has no `-a` equivalent — for any search targeting `main.ts`, run `grep -a`/`rg -a` directly via `Bash` instead.
