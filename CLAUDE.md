...
>
> **Thin features (temples, chains, fingers, accessory straps) fray or fragment under a coarse march — mesh them at their own fine resolution, don't rely on the global `edgeLength`.** A coarse march skeletonizes anything thinner than its grid; the fix is a per-region fine mesh (a tighter `edgeLength` scoped to just that primitive, or overlapping `smoothUnion` capsules along the feature regardless of the global setting), not a blanket resolution bump. And if `genus` *rises* as you refine further, that's a different problem — a non-Lipschitz `smoothUnion`/`smin` artifact, not under-resolution — so don't burn a pass "fixing" it with yet more resolution.
>
...
> **A shared figure/garment builder fix does NOT retroactively update the baked catalog — re-bake explicitly, every time.** Catalog entries (`public/catalog/*.partwright.json`) are a **snapshot**: source + mesh + thumbnail from whenever they were last baked. Fixing a shared function (e.g. in `sdfFigure.ts`, a garment builder) changes what *new* runs produce but leaves every already-baked entry showing the old — possibly still-buggy — geometry until it's re-baked. After any shared-builder change, find every catalog entry whose embedded `code` calls the changed function and re-bake each one — `scripts/rebake-figure-catalog.cjs` auto-discovers entries by source pattern (e.g. `code.includes('sdf.figure')`), and `scripts/rebake-shod-figures.cjs` is the footwear-specific precedent. Don't rely on a user noticing a stale thumbnail — this exact class of "fixed the code but shipped the old bake" regression has recurred across many sessions.
