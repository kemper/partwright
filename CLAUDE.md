...
- **The dedicated `Grep` tool has no `-a` flag** — it can't search `main.ts` at all. Go straight to `Bash` with `grep -an` (or `rg -an`) instead of trying `Grep` first and falling back.
...
If a grep on `main.ts` returns nothing for a symbol you expect to find there, binary-detection is the first thing to check. Four independent sessions have each spent turns re-discovering this — one after already burning a wasted `Grep -a` attempt because the tool silently doesn't support the flag.
