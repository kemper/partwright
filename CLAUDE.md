...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a`/`rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool has no `-a` flag** — reach for Bash's `grep -an` directly rather than trying `-a` on the `Grep` tool first (it's a no-op there and just wastes a call).
...
| A Worker-backed pool/client hangs forever with no error | Missing the `{type:'init'}` → `ready` handshake before the first `execute` message. `engineWorker.ts` returns `{type:'error', message:'Geometry engine not initialised'}` for an un-initialized worker's `execute` — but a fresh pool worker that skips `init` never gets that far, so a caller that only awaits a resolved `execute` (never handling the `error` message type) hangs indefinitely instead of rejecting. Any new Worker client must send `init`, await `ready`, and handle `error`. |
