...
**Before renaming a user-visible string** (button label, tab name, modal title) that e2e specs reference by text: run `grep -rn '"old label"' tests/` to find test selectors, update them before pushing, and re-run the grep after every merge from main. A label rename silently breaks test selectors in two separate CI rounds if not caught early.
...
**`scripts/build-catalog-entry.cjs`** (single-entry re-bake with real WebGL thumbnails) launches a **headed** Chromium and fails in this container without a display — run it as `xvfb-run -a node scripts/build-catalog-entry.cjs ...`. `scripts/catalog-regen.cjs` already uses `headless: true` and doesn't need this wrapper.
...
For the full Manifold/CrossSection API, `window.partwright` console API, session workflow, verification patterns, and photo-to-model workflow, see `public/ai.md`. The legacy `window.mainifold` alias remains available for older prompts. **Before designing a new `api.*` sandbox namespace, search `public/ai.md` for existing ones** — the sandbox already exposes `api.sdf`, `api.paint`, `api.surface`, `api.printFit`, `api.fasteners`, `api.joints`, and others. Discovering at mid-task that 80% of a planned namespace already exists has happened.
...
- `src/surface/modifiers.ts` — Surface modifier pipeline (`SurfaceModifierId = 'fuzzy' | 'knit' | 'cable' | 'waffle' | 'fur' | 'woven' | 'voronoi' | 'voronoiLamp' | 'smooth' | 'voxelize'`): `applyFuzzy` (noise-displaced skin), the fabric-texture family `applyKnit` / `applyCable` / `applyWaffle` / `applyFur` / `applyWoven` (stockinette knit, cable knit, waffle stitch, fur/velvet, woven fabric — displaced along normals over a UV unwrap, with WebGPU compute where available), `applyVoronoi` (Voronoi cell relief), `applyVoronoiLamp` (perforated SDF lamp shell), `applySmooth` (Taubin smoothing pass), `applyVoxelize` (mesh → voxel grid), `applyScale` (non-destructive resize). Most modifiers also have an `apply*Patch` variant that textures only a selected triangle set. Each returns a `ModifierResult` — either `'manifold'` (baked mesh + wrapper code, mirroring the STL import path) or `'voxel'` (encoded grid + inline `voxels.decode(…)` code). Pure math lives in sibling modules: `fuzzySkin.ts`, `knitTexture.ts`, `knitTextureGPU.ts`, `cableKnit.ts`, `waffleStitch.ts`, `furVelvet.ts`, `wovenFabric.ts`, `voronoiShell.ts`, `voronoiLattice.ts`, `voronoiLampSdf.ts` (over the `sdfModifier.ts` scaffolding), `smoothSurface.ts`, `voxelizeMesh.ts`, `meshSubdivide.ts`, `colorTransfer.ts`, `scaleMesh.ts`, plus the UV layers `uvParameterize.ts`, `uvUnwrap.ts`, and `placement.ts` (region placement). Unit tests: `tests/unit/surface.test.ts`.
- `src/surface/surfaceOps.ts` + `surfaceOpSpec.ts` — the **in-code** (non-baking) surface-texture path: manifold-js model code declares textures via `api.surface.*` (`fuzzy`/`knit`/`cable`/`waffle`/`fur`/`woven`/`voronoi`/`smooth` — the mesh-producing subset; `voxelize`/`voronoiLamp` change engines so they stay bake-only). The Worker records the validated op chain (`surfaceOpSpec.ts` is the dependency-free shared spec — its option allow-lists are effectively **append-only** once user code persists them, since unknown keys throw), `surfaceOps.ts` applies it main-side reusing the modifier math, memoized per chain prefix. Explicit runs (`run`/`runAndSave`/Run button/version loads) force-apply; live-typing auto-runs show the base mesh plus a sticky "⟳ Re-apply" pill. Computed textures **persist on saved versions** (`Version.surfaceTexture` = full-chain memo key + textured mesh, export schema 1.14): a version load seeds the memo cache so a reopened session renders textured instantly with no recompute, pinning the texture's appearance at save time — the key folds in code + params + import identity, so a stale key just recomputes (never renders the wrong texture). The **Surface panel writes this path too**: in a manifold-js session with whole-model mode, Apply becomes "Apply as code" and upserts the `api.surface.<id>({…})` call via `src/surface/surfaceCodegen.ts` + `partwright.applySurfaceTextureAsCode` (region/patch applies, voxelize/voronoiLamp, and SCAD/BREP sessions keep the bake path). The sibling `api.paint.*` (recorded in `engines/manifoldJs.ts`, resolved into the model-color underlay in `src/color/regions.ts`) declares paint in code the same way. Unit tests: `tests/unit/surfaceOps.test.ts`, `tests/unit/surfaceCodegen.test.ts`; e2e: `tests/surface-in-code.spec.ts`, `tests/surface-panel-as-code.spec.ts`, `tests/paint-in-code.spec.ts`.
...
1. **Subscribe and watch CI.** Call `subscribe_pr_activity`. PR-checks runs build + unit + 3 e2e shards on every push, draft or ready — don't flip to ready to trigger it. Fix failures on the branch (each push re-runs the suite); fall back to local `npm run test:e2e` only when iterating tight on a CI failure. **If pr-checks stops firing after a push while push-triggered workflows (CodeQL, Cloudflare) keep running, check mergeability first** — a conflicted PR silently skips all `pull_request`-event CI; `git merge-tree origin/main HEAD` (exits non-zero on conflict) diagnoses it in seconds.
