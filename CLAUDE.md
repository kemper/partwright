...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a` or `rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool has no `-a` flag** (its schema only exposes `-A`/`-B`/`-i`/`-n`/`-o`/`context`/`glob`/etc.) — reach for `Bash` (`grep -an '<pattern>' src/main.ts`) instead, not a `Grep` tool call with an unsupported flag.
