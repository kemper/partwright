...
>
> **Measure geometry empirically when `smoothUnion` is involved — don't trust analytic primitive extents, and never assert envelopes from `bounds()`.** A `smoothUnion` bulges the surface *past* either input primitive's bounds (a foot's real underside sat ~0.79·r below the sole centre vs the analytic 0.65·r), and `bounds()` is a conservative loose AABB — the real surface can be significantly inside or outside it. **Never write test assertions against `bounds()` output; use `evaluate(x,y,z)` to find the actual surface.** Walk `evaluate` along the axis in a tiny vite-node script (`npx vite-node probe.mjs` importing `__figureTestables__` + `__testables__`) until the sign flips. Set clearances/clip planes below the *measured* value, then confirm coverage with a sample-grid check (sample where one label is solid; assert the covering label is solid there too).
>
...
> **Headless `componentCount` can under-report vs the browser bake for near-threshold thin features.** The Node (headless) mesher bridges small gaps that the browser's WebGL-threaded mesher leaves as separate islands. For catalog figures with thin/proud features (braids, cords, displaced-texture cords), a headless `componentCount: 1` does NOT guarantee the browser bake is also 1 — verify via `xvfb-run -a node scripts/build-catalog-entry.cjs` before committing. Use headless `componentCount` for mechanism mechanisms (clearance splits) where both meshers agree, and as an early-pass filter — but treat it as advisory for near-threshold geometry.
