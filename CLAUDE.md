...
- **`grep`/`rg` silently truncate results** or skip the file — search it with `grep -a`/`rg -a` **via Bash**, not the Grep tool (its wrapper has no `-a` flag; passing one is a no-op that reproduces the truncation).
