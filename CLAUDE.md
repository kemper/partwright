...
- **Paint-in-code is verified headlessly.** `api.paint.*` ops (box/slab/cylinder/label) resolve against the mesh with the same pure helpers the browser uses: the PNG shows the colours and `stats.paintOps` lists per-op `{name, kind, triangleCount}` — an op that resolves to **0 triangles warns** (region misses the surface / label doesn't exist). Brush-painted sidecar regions still need the browser. **Flush-surface paint dithers:** a decorative region (areola, eyebrow, skin marking) sitting exactly flush on its surround produces salt-and-pepper edges in the colored bake — nearest-centroid label assignment can't disambiguate coincident surfaces. Fix: add ~1 triangle of outward relief so the painted layer is slightly proud. `stats.paintOps` shows non-zero triangles (no warning fires), but the bake result still dithers — only the colored bake reveals this.
...
>
> **Catalog `.partwright.json` files are a snapshot — they go stale whenever source changes.** A code fix to a figure builder or any shared SDF helper it calls does **not** propagate to the catalog until you explicitly re-bake the affected entries. When you modify a shared helper (torso, foot, garment, grasp, etc.), treat every catalog figure that calls it as stale and re-bake before shipping. There is no CI freshness gate today (see backlog).
