...
- **OpenAI (cloud)** — `src/ai/openai.ts`. Raw `fetch` with SSE streaming; no extra SDK. The agent loop (`streamTurn`) runs on the **Responses API** (`/v1/responses`) because gpt-5.5+ reject `reasoning_effort` alongside function tools on `/v1/chat/completions` and direct callers to `/v1/responses`; Responses supports the 4o/4.1 chat models too, so the whole loop runs through it. History converts to the Responses `input` shape (`message`/`function_call`/`function_call_output` items linked by `call_id`); the non-tool, non-reasoning helpers (`validateKey`/`listModels`/`summarize`) stay on `/v1/chat/completions`.
...
- **OpenAI** — maps to the Responses `reasoning.effort` (`low/medium/high`), sent only for reasoning models (`gpt-5*`, `o1/o3/o4` — sniffed by `isReasoningModel`) so the 4o/4.1 chat models don't 400. 'off' omits the `reasoning` field. OpenAI hides reasoning-model CoT, so this controls cost/quality but never surfaces a thinking box.
