...
3. **Pre-flight, then push a draft PR.** Re-sync with the latest main (`git fetch origin main`, then merge `origin/main` into your branch, or rebase onto it if the branch hasn't been pushed yet, resolving conflicts), run the fast `npm run build` + `npm run test:unit` to catch type errors and logic regressions, push the branch, and open the PR into `main` **as a draft** (`create_pull_request` with `draft: true`). Keep the pre-flight light — build + unit only. The PR-checks CI (`.github/workflows/pr-checks.yml`) runs build + unit on every PR push for fast feedback; the sharded `npm run test:e2e` suite is **opt-in via the `run-e2e` label**, which you'll add in the next step once the implementation looks good (this keeps shard minutes off draft pushes that aren't ready for full verification). See [Pull Requests](#pull-requests--open-a-draft-when-the-work-looks-good-mark-ready-once-verified).
4. **Add `run-e2e`, then verify on the draft.** Once build + unit go green and your own light checks (render/stat verification, code review of the diff) look good, **add the `run-e2e` label** to kick off the full e2e suite in CI: `issue_write` with `method: "update"`, `issue_number: <PR>`, `labels: ["run-e2e"]` — note that `labels` *replaces* the label set, so include any other labels already on the PR. Adding the label fires a fresh workflow run that includes the 3 e2e shards. Subscribe to PR activity, follow the e2e shards, and run any deeper or manual verification alongside CI. Fix failures on the same branch until every check is green. Only fall back to local `npm run test:e2e` if you need a tight loop on a failure CI surfaced. See [After Opening a PR](#after-opening-a-pr).
5. **Mark ready for review** once the review pass is clean and PR-checks CI is green (build + unit + every e2e shard) (`update_pull_request` with `draft: false`). The task is not done until the PR is out of draft.
...
**Workflow labels** (don't affect release notes; gate CI behavior):

- `run-e2e` — when present on a PR, PR-checks runs the sharded e2e suite. Add this once the implementation looks good and you want the full e2e signal before flipping the PR to ready. Adding/removing the label triggers a fresh workflow run. Remember `issue_write`'s `labels` field *replaces* the label set, so include any existing labels when adding `run-e2e` (and vice versa when adding release-note labels later).
...
**1. Add `run-e2e`, then let CI run the slow verification you skipped pre-flight.** PR-checks runs the full e2e suite (sharded 3-way) only when the PR carries the `run-e2e` label, so once build + unit are green and your own light verification looks good, add the label to fire the e2e shards: `issue_write({ method: "update", owner, repo, issue_number: <PR>, labels: ["run-e2e", ...anyExistingLabels] })`. The `labels` field *replaces* the label set, so always include any labels already on the PR (read them via `pull_request_read` if you're not sure). Subscribe to PR activity (`subscribe_pr_activity`) and watch the e2e shards rather than running e2e locally up front. Run any deeper or manual verification the change warrants — e.g. exercising the affected UI in a browser per the [Smoke Test](#smoke-test--verifying-the-app-works) — alongside CI. Fall back to local `npm run test:e2e` only when iterating tight on a failure CI surfaced. Fix failures on the same branch until every PR-checks shard goes green.
