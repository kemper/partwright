...
- **`grep`/`rg` silently truncate results** or skip the file — use `grep -a` or `rg -a` for any search targeting `main.ts`. **The dedicated `Grep` tool has no `-a`/binary-override flag** — passing one is a wasted call. Go straight to `Bash` with `grep -an` (or `rg -a`) for any search that touches `main.ts`.
...
If a grep on `main.ts` returns nothing for a symbol you expect to find there, binary-detection is the first thing to check. Four independent sessions have each spent turns re-discovering this (one after that, re-discovering that the fix requires `Bash`, not the `Grep` tool).
