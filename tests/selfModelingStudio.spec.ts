...
test.describe('Self-Modeling Studio', () => {
  ...
  test('open → upload angles → hand off to AI (references + brief) → reopen prefilled', async ({ page }) => {
    ...
    // Default angle set is Portrait (5 front-hemisphere views).
    await expect(page.getByText(/0 of 5 views ready/)).toBeVisible();
    ...
    // Critically: the references reach the CHAT (a pending image that rides into
    // the model's vision on send), not just the gallery — as ONE labeled grid of
    // all the views (not N separate images). A fresh transcript has no other
    // images, so there's exactly one pending chip.
    await expect(page.locator('#ai-panel img')).toHaveCount(1);
