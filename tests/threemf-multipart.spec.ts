...
test.describe('multi-part 3MF export', () => {
  ...
  test('printer selection swaps the base + stamps identity/bed (H2C dual vs P1S single)', async ({ page }) => {
    ...
    const out = await page.evaluate(async () => {
      ...
      // A1 is single-nozzle and a non-base printer (regression guard: a non-base
      // single-nozzle printer must use the P1S base AND carry its own process);
      // H2D is dual + a non-base printer (guards the print-process compatibility fix).
      const a1 = await decode(build3MFProject(parts, { bambu: true, printer: 'a1' }));
      ...
      return {
        ...
        a1Nozzles: arrLen(a1, 'nozzle_diameter'),
        a1Process: str(a1, 'print_settings_id'),
        a1Compat: /"print_compatible_printers":\s*\[([^\]]*)\]/.exec(a1)?.[1],
    ...
    // A1 is SINGLE-nozzle (uses the P1S base, not the dual H2C base) — regression guard.
    expect(out.a1Nozzles).toBe(1);
    ...
    expect(out.a1Process).toBe('0.20mm Standard @BBL A1');
    expect(out.a1Compat).toContain('Bambu Lab A1');
